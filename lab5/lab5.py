...
def most_misclassified(classifier, n=5):
    ...
    weights = [ (i,classifier.data_weights[i]) for i in xrange(n)]

    sortedWeights = sorted(weights, key=lambda l : l[0] )
    sortedWeights.reverse()

    output = [ legislator_info(classifier.data[index]) for index in [ x[0] for x in sortedWeights] ]

    return output

    # return [ legislator_info(datum) for datum in classifier.data[0:n-1] ]
...
boost.train(20)
# print 'classifiers for 110th senate\n'
# for classifier in boost.classifiers:
#     print classifier,'\n'
